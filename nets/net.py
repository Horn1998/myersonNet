...
class MyersonNet():
    ...
    def nn_build(self):
        ...
        self.x  = tf.compat.v1.placeholder(tf.float32, [None, num_agent])
    ...
    def nn_eval(self, str): 
        ...
        # 每个用户的输入是相同的
        spa_tensor1 = tf.reshape(tf.tile(tf.reshape(vv, [-1]), [num_agent]),[num_agent, -1, num_agent])
        #将当前对象对应的值设为0，然后寻找最大值（见论文）
        spa_tensor2 = tf.matmul(spa_tensor1, tf.linalg.diag(w_p))
        #z找到当前角色除自身之外的最大虚拟估值
        p_spa = tf.transpose(tf.reduce_max(spa_tensor2, reduction_indices=[2]))
        
        ## Decode the payment 计算反函数
        ...
        p_max_units = tf.reduce_min(tf.multiply(p_spa_copy - w_encode2_copy,\
                        tf.math.reciprocal(tf.exp(w_encode1_copy))),\
                            reduction_indices=[2])
    ...
    def nn_test(self, data, mechanism):
        ...
        num_agent                        = self.args.num_agent
        data_size                        = sample_val.shape[0]
        # 主要解决小于0的情况，如果虚拟估值均大于0，则直接计算最大值即可
        ...
        for i in range(num_recordings):
            rev_array[i], alloc_error_array[i], vv_array =\
                self.sess.run((self.revenue_test, a_error, self.vv_test),
                    feed_dict={self.w_encode1: w_encode1_array[i,:,:,:], self.w_encode2: w_encode2_array[i,:,:,:],self.x: sample_val})
